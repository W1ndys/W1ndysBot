...
class DataManager:
    ...
    def _migrate_old_data(cls):
        ...
        assert cls._conn is not None  # 添加断言确保连接存在
        try:
            data_base_dir = os.path.join("data", MODULE_NAME)

            # 查找所有旧的群组数据目录
            pattern = os.path.join(data_base_dir, "*", "data.db")
            old_db_files = glob.glob(pattern)

            if not old_db_files:
                print(f"[{MODULE_NAME}] 没有找到需要迁移的旧数据库文件")
                return

            print(
                f"[{MODULE_NAME}] 开始迁移旧数据库文件，找到 {len(old_db_files)} 个文件"
            )
            ...
            for old_db_path in old_db_files:
                try:
                    # 从路径中提取群组ID
                    group_id = os.path.basename(os.path.dirname(old_db_path))
                    ...
                    try:
                        # 检查旧表结构
                        old_cursor.execute("PRAGMA table_info(ban_words)")
                        ban_words_columns = [row[1] for row in old_cursor.fetchall()]

                        if "update_time" in ban_words_columns:
                            old_cursor.execute(
                                "SELECT word, weight, update_time FROM ban_words"
                            )
                            ban_words_data = old_cursor.fetchall()
                            for word, weight, update_time in ban_words_data:
                                cursor.execute(
                                    """
                                    INSERT OR IGNORE INTO ban_words (group_id, word, weight, update_time) 
                                    VALUES (?, ?, ?, ?)
                                    """,
                                    (group_id, word, weight, update_time),
                                )
                        else:
                            old_cursor.execute("SELECT word, weight FROM ban_words")
                            ban_words_data = old_cursor.fetchall()
                            for word, weight in ban_words_data:
                                cursor.execute(
                                    """
                                    INSERT OR IGNORE INTO ban_words (group_id, word, weight) 
                                    VALUES (?, ?, ?)
                                    """,
                                    (group_id, word, weight),
                                )

                        migrated_words = len(ban_words_data)
                        print(
                            f"[{MODULE_NAME}] 群组 {group_id} 迁移了 {migrated_words} 个违禁词"
                        )

                    except sqlite3.OperationalError as e:
                        if "no such table: ban_words" not in str(e):
                            print(
                                f"[{MODULE_NAME}] 迁移群组 {group_id} 违禁词时出错: {e}"
                            )

                    # 迁移用户状态数据
                    try:
                        # 检查旧表结构
                        old_cursor.execute("PRAGMA table_info(user_status)")
                        user_status_columns = [row[1] for row in old_cursor.fetchall()]

                        if "update_time" in user_status_columns:
                            old_cursor.execute(
                                "SELECT user_id, status, update_time FROM user_status"
                            )
                            user_status_data = old_cursor.fetchall()
                            for user_id, status, update_time in user_status_data:
                                cursor.execute(
                                    """
                                    INSERT OR IGNORE INTO user_status (group_id, user_id, status, update_time) 
                                    VALUES (?, ?, ?, ?)
                                    """,
                                    (group_id, user_id, status, update_time),
                                )
                        else:
                            old_cursor.execute(
                                "SELECT user_id, status FROM user_status"
                            )
                            user_status_data = old_cursor.fetchall()
                            for user_id, status in user_status_data:
                                cursor.execute(
                                    """
                                    INSERT OR IGNORE INTO user_status (group_id, user_id, status) 
                                    VALUES (?, ?, ?)
                                    """,
                                    (group_id, user_id, status),
                                )

                        migrated_users = len(user_status_data)
                        print(
                            f"[{MODULE_NAME}] 群组 {group_id} 迁移了 {migrated_users} 个用户状态"
                        )

                    except sqlite3.OperationalError as e:
                        if "no such table: user_status" not in str(e):
                            print(
                                f"[{MODULE_NAME}] 迁移群组 {group_id} 用户状态时出错: {e}"
                            )
                    ...
                    print(f"[{MODULE_NAME}] 迁移数据库 {old_db_path} 时出错: {e}")
                    continue
            ...
            if migrated_count > 0:
                print(
                    f"[{MODULE_NAME}] 数据迁移完成，成功迁移 {migrated_count} 个群组的数据"
                )

                # 询问是否删除旧数据库文件
                cls._cleanup_old_data(old_db_files)

        except Exception as e:
            print(f"[{MODULE_NAME}] 数据迁移过程中出现错误: {e}")

    @classmethod
    def _cleanup_old_data(cls, old_db_files):
        """清理旧数据库文件（可选）"""
        try:
            # 创建备份目录
            backup_dir = os.path.join("data", MODULE_NAME, "backup_old_data")
            os.makedirs(backup_dir, exist_ok=True)

            for old_db_path in old_db_files:
                group_id = os.path.basename(os.path.dirname(old_db_path))
                backup_path = os.path.join(backup_dir, f"{group_id}_data.db")
                ...
                # 删除空的群组目录
                group_dir = os.path.dirname(old_db_path)
                try:
                    os.rmdir(group_dir)
                except OSError:
                    # 目录不为空，保留
            ...
            print(f"[{MODULE_NAME}] 旧数据库文件已移动到备份目录: {backup_dir}")

        except Exception as e:
            print(f"[{MODULE_NAME}] 清理旧数据时出错: {e}")
